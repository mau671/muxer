...
def _show_track_summary(tracks: List[Dict[str, Any]]) -> None:
    ...
    for track in audio_tracks:
        ...
        default = " (default)" if track['properties'].get('default_track', False) else ""
...
def mux_files(input_file: str, output_file: str, tracks: List[Dict[str, Any]]) -> None:
    ...
    command = ["mkvmerge", "--ui-language", "en_US", "-v", "-o", output_file]  # Added --ui-language for English output
    track_order = []
    subtitle_tracks = []
    audio_tracks = []
...
def _execute_with_progress(command: List[str], output_file: str) -> None:
    ...
    with Progress(
        ...
        if return_code != 0:
            console.print(f"[red]❌ Error executing mkvmerge with return code {return_code}[/red]")
            sys.exit(1)
